...
Em primeiro lugar, é preciso compreender que o mercado financeiro é um espaço institucionalizado no qual diversos agentes econômicos negociam diferentes tipos de ativos, como títulos de dívida, commodities, derivativos e moedas \cite{MishkinEakins2018}. 
...
É comum ouvir falar em \textbf{mercado de baixa (bear market)} e \textbf{mercado de alta (bull market)}. O primeiro descreve períodos prolongados de queda nos preços das ações e pessimismo generalizado no mercado. Por outro lado, um \textbf{mercado de alta (bull market)} refere-se a fases caracterizadas por aumentos contínuos e generalizados dos preços dos ativos financeiros, acompanhados por otimismo dos investidores em relação ao futuro econômico \cite{BodieKaneMarcus2021}.
...
Além dos índices de mercado, existem os chamados indicadores técnicos, muito usados na análise gráfica para entender o comportamento dos preços e subsidiar decisões de compra e venda. Diferentemente dos índices, que refletem o desempenho de um conjunto de ativos (como o Ibovespa), os indicadores técnicos são aplicados a cada ativo em separado, com base em cálculos matemáticos sobre preços históricos e volumes de negociação, gerando sinais que auxiliam o investidor \cite{Murphy1999}.
...
\begin{itemize}
  ...
  \item {\textit{spread} bid–ask}, diferença entre o maior preço que alguém está disposto a pagar por um ativo e o menor preço pelo qual alguém está disposto a vendê-lo;  
...
Para avaliar corretamente esse \textit{trade-off} e estimar a performance out-of-sample, empregam-se técnicas de validação temporal avançada, tais como \textit{time series cross-validation} com bloqueio por períodos e backtesting \textit{walk-forward}, que previnem o viés de uso de informação futura (\textit{look-ahead bias}) e fornecem indicadores realistas de acurácia \cite{bergmeir2012}. Adicionalmente, modelos de mudança de regime, como o proposto por Hamilton (1989), capturam transições abruptas entre fases de mercado (e.g., bull vs. bear), ajustando a sensibilidade do algoritmo conforme o regime vigente \cite{hamilton1989switching}.  
...
Para superar essa limitação, Hochreiter e Schmidhuber \cite{hochreiter_1997} propuseram a arquitetura Long Short-Term Memory (LSTM), uma variante sofisticada de RNN. A principal inovação da LSTM é sua unidade de memória, composta por um estado de célula (cell state) e três portões (gates) que regulam o fluxo de informação. O estado de célula atua como uma esteira rolante, transportando informações relevantes ao longo da sequência com poucas alterações. Os portões são estruturas neurais que, por meio de funções de ativação sigmoide, decidem qual informação deve ser adicionada ou removida do estado de célula. A função sigmoide é uma curva em formato de "S" que transforma qualquer valor numérico em um número entre 0 e 1, funcionando como uma espécie de "interruptor gradual": valores muito negativos se aproximam de 0, valores muito positivos se aproximam de 1, e valores próximos de zero ficam em torno de 0,5. Essa característica torna a sigmoide ideal para representar probabilidades ou proporções, permitindo que os portões da LSTM "decidam" quanto de informação manter ou descartar de forma suave e diferenciável.
...
\caption{A arquitetura do modelo Transformer, destacando os blocos de codificador e decodificador, as camadas de atenção multi-cabeças e as redes feed-forward. Fonte: Adaptado de Vaswani \textit{et al.} (2017) \cite{vaswani2017attention}.}
\label{fig:arquitetura-transformer}
\end{figure}
