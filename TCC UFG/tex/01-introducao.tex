...
\item \textbf{Comparadores (modelos de referência)}: Contrastamos o modelo proposto com referências clássicas e modernas: 
\begin{enumerate}[label=\Roman*.]
...
\begin{quote}
Em que condições (janelas, regimes de volatilidade e custo) um modelo CNN+LSTM supera comparadores clássicos (Naive, ARIMA, Prophet) e variantes com atenção na tarefa de previsão direcional intradiária (+1 barra/15 min) em algumas ações líquidas da B3 (2020–2025), sob avaliação temporal deslizante (walk-forward) e métricas pós-custos?
...
Em segundo lugar, ao realizar uma avaliação empírica comparativa com modelos de referência (baselines) estabelecidos (ARIMA, Prophet, LSTM puro e variantes Transformer), sob um protocolo realista de validação temporal deslizante (walk-forward) e testes retrospectivos (backtests) pós-custos. 
