...
No horizonte intradiário, emergem problemas clássicos de microestrutura — periodicidades sistemáticas ao longo do dia, espaçamento irregular entre negócios e impacto de ordens (inclusive via spread\footnote{Diferença entre o preço de compra (bid) e o de venda (ask) de um ativo.} e custo de imediatismo) — que condicionam a dinâmica de preços e a interpretação de sinais de curto prazo \cite{andersen1997intraday,engle1998acd,hasbrouck2007empirical,amihud2002illiquidity,dufour2000time}. Além disso, choques macroeconômicos e eventos exógenos podem deslocar rapidamente as cotações, elevando a incerteza \cite{WorldBank2020}. Esse contexto motiva a investigação de métodos preditivos sensíveis ao tempo e às nuances operacionais do ambiente de negociação brasileiro.
...
Portanto, este trabalho avança a linha dos híbridos ao empregar um modelo CNN+LSTM com camadas convolucionais para padrões locais, validação temporal deslizante (walk-forward) e uma avaliação realista pós-custos no contexto da B3, com foco em estabilidade e reprodutibilidade dos resultados.
...
\begin{itemize}
    ...
    \item \textbf{Validação e treinamento}: Para evitar viés temporal (look-ahead) e superestimação de desempenho, adotamos validação temporal deslizante (walk-forward) com janelas sequenciais, seleção de hiperparâmetros por otimização Bayesiana e controle de sementes/versionamento para reprodutibilidade. Esse arranjo segue boas práticas de validação em séries temporais e testes retrospectivos (backtesting) \cite{bergmeir2012,lopezdeprado2018}.
...
\begin{enumerate}[label=\Roman*.]
    ...
    \item métricas pós-custos de negociação (trading) (retorno líquido, queda máxima (drawdown) e índice de Sharpe), refletindo utilidade prática sob custos de transação \cite{sharpe1994}
\end{enumerate}
Para comparar previsores quando necessário, usamos testes de acurácia preditiva como Diebold–Mariano \cite{diebold1995}.


\item \textbf{Comparadores (modelos de referência)}: Contrastamos o modelo proposto com os seguintes baselines, efetivamente avaliados nos experimentos: 
\begin{enumerate}[label=\Roman*.]
    \item Naive e Drift, como pisos de desempenho (regras simples sem modelo);
    \item ARIMA/Box–Jenkins como modelo de referência (baseline) estatístico canônico \cite{box_2015,hyndman_2018,rotela2014arima}; 
...
Avaliar de forma quantitativa e reprodutível, a capacidade de modelos de aprendizado de máquina sensíveis ao tempo em prever o comportamento direcional de indicadores financeiros intradiários, comparando seu desempenho a modelos de referência (baselines) estatísticos e neurais sob um protocolo experimental realista (validação temporal deslizante (walk-forward) e teste retrospectivo (backtest)\footnote{Simulação histórica de uma estratégia sob regras e dados passados.}) pós-custos.
...
\begin{enumerate}
  ...
  \item Definir o desenho experimental com validação temporal deslizante (walk-forward) (janelas deslizantes, embargo temporal e padronização dentro de cada janela), estratégia de seleção de hiperparâmetros e controle de variáveis (sementes, versões e ambiente).
  \item Treinar e comparar modelos de referência (baselines) representativos com o(s) modelo(s) proposto(s) sensíveis à ordem temporal, mantendo critérios comparáveis de otimização.
  ...
  \item Avaliar desempenho operacional convertendo sinais em regras simples de negociação e realizando testes retrospectivos (backtests) com custos de transação e deslizamento de preço (slippage), reportando retorno, Sharpe, queda máxima (max drawdown) e rotatividade (turnover), além de cortes por regimes de volatilidade.
  \item Testar significância e robustez dos resultados por meio do teste de Diebold-Mariano, bootstrap em blocos e análises de sensibilidade (janelas temporais, subconjuntos de ativos, horizonte de previsão e conjuntos de características (feature sets)).
...
Em segundo lugar, ao realizar uma avaliação empírica comparativa com modelos de referência (baselines) estabelecidos (Naive, Drift, ARIMA e Prophet), sob um protocolo realista de validação temporal deslizante (walk-forward) e testes retrospectivos (backtests) pós-custos. 
...
Por fim, o trabalho inclui um pipeline reprodutível, com código-fonte organizado e documentação detalhada, para apoiar a replicação dos experimentos e a adaptação ou extensão por outros pesquisadores. O material disponibilizado reduz barreiras técnicas e favorece a co-construção do conhecimento; a natureza aberta do pipeline permite que novas metodologias sejam testadas em condições equivalentes, tornando os resultados mais comparáveis e contribuindo para conclusões mais robustas.
