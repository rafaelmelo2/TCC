...
A literatura sobre previsão (forecasting) de séries temporais financeiras é vasta, com abordagens que evoluíram dos modelos estatísticos clássicos para arquiteturas híbridas integrando aprendizado de máquina (machine learning), redes neurais e indicadores técnicos.
...
\begin{figure}[H] % h=aqui, t=topo, b=base, p=página de floats, !=relaxa limites
  ...
  \includegraphics[width=0.6\linewidth]{validacao-temporal-walk-forward.png}
    \caption{Validação temporal deslizante (walk-forward) com seleção bayesiana de hiperparâmetros. Fonte: autor.}
...
Além disso, trabalhos como LLM4TS (Chang et al., 2023) apresentaram esquema de alinhamento e ajuste fino adaptativo (fine-tuning) de LLMs pré‑treinados para séries temporais, alcançando redução média de ~6,8\% no MSE em cenários de aprendizado com poucos exemplos (few-shot) em comparação com modelos treinados do zero \cite{chang2023llm4ts}. Jin et al. (2023) criaram a arquitetura Time‑LLM, que reprograma LLMs em tarefas de previsão (forecasting) unificando prompts como prefixos, e demonstrou desempenho competitivo frente a modelos especializados, inclusive em cenários de inferência sem ajuste (zero-shot) \cite{jin2023time_llm}.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\bfseries}l Y}
    ...
    Dados para ambos os blocos; etapas de decomposição e resíduos; pipeline mais complexo. \\
    LLMs s/ séries (Time-LLM, LLM4TS, Zero-shot) &
    Pré-processo conversão tempo-para-texto (time-to-text)/prompting; poucos dados funcionam (few/zero-shot); sem treino pesado. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\bfseries}l Y}
    ...
    \midrule
    Modelos estatísticos (ARIMA/ARIMAX) & Interpretável; bom curto prazo; modelo de referência (baseline) sólido. \\
...
%     LSTM &
%     Boa em ativos líquidos (minutos--diário) com validação temporal deslizante (walk-forward) e regularização. \\
%     Transformers (PatchTST, Non-stationary) &
