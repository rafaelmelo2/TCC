...
% ============================================================================
Este capítulo detalha os procedimentos adotados para desenvolver e avaliar o modelo proposto. A metodologia está organizada nas Seções 4.1 a 4.8, cobrindo: aquisição e tratamento de dados, pré-processamento e engenharia de atributos, modelos (modelos de referência (\textit{baselines}) e proposto), desenho experimental e treinamento, métricas de avaliação, testes retrospectivos (\textit{backtests}) com custos, testes de robustez e significância e, por fim, diretrizes de reprodutibilidade.
% ---------------------------------------------------------------------------
% Figura: \textit{pipeline} completo de construção e avaliação do modelo
% % ---------------------------------------------------------------------------
...
%   \caption{Pipeline completo de construção e avaliação do modelo: desde a aquisição de dados brutos até a consolidação dos resultados, incluindo validação \textit{walk-forward}, treinamento de \textit{baselines} e modelo proposto (CNN-LSTM), avaliação com métricas direcionais e probabilísticas, \textit{backtests} com custos reais e testes estatísticos de significância. Fonte: o autor.}
...
Os dados foram obtidos via plataforma MetaTrader~5 (MT5), com histórico consolidado e marcação temporal (\textit{timestamping}) consistente no fuso do pregão \cite{b3_servicos}. Para utilizar o MT5, basta ter conta na corretora Clear, que disponibiliza a plataforma gratuitamente. A escolha dos ativos não foi arbitrária: o universo contempla \textbf{três ativos} que figuram entre os de maior relevância e volume de negociação na B3 — PETR4 (Petrobras), VALE3 (Vale) e ITUB4 (Itaú Unibanco). Esses papéis estão entre os mais líquidos e mais movimentados da bolsa brasileira, com participação relevante no índice Ibovespa e em indicadores de volume, o que os torna adequados para estudos intradiários: além de reduzir spreads, gaps e deslizamento de preço (\textit{slippage}), ativos mais movimentados tendem a oferecer séries temporais mais estáveis e menos sujeitas a artefatos de microestrutura em testes retrospectivos (\textit{backtests}). Os ativos foram mantidos fixos durante todo o período para mitigar viés de sobrevivência (survivorship bias).

% --- Subseção: Recorte temporal e granularidade
\subsection{Recorte temporal e granularidade}
Foram utilizadas barras de \textbf{15 minutos} (período (\textit{timeframe}) M15). O período efetivo compreende \textbf{22 de outubro de 2020} a \textbf{22 de outubro de 2025} (cinco anos), condicionado pela disponibilidade de histórico no MT5 para a granularidade selecionada \cite{andersen1997intraday}.

% --- Subseção: Variáveis
\subsection{Variáveis}
Cada barra contém as variáveis padrão de preço e volume: \textbf{OHLCV} --- abertura (open), máxima (high), mínima (low), fechamento (close) e volume em contratos (volume\_real). Incluem-se ainda volume\_ticks (número de ticks por barra) e \textit{spread} (diferença bid-ask na marcação temporal (timestamp)), empregadas em checagens de qualidade e na análise de microestrutura. As características (features) adicionais (retornos logarítmicos, volatilidade móvel, MME, RSI, Bandas de Bollinger) são construídas na etapa de engenharia de atributos (Seção~4.2).
...
clássicos em séries financeiras, especialmente o vazamento de dados (\textit{data leakage}).
...
\subsection{Normalização e prevenção de vazamento de dados (\textit{data leakage})}
As características (features) numéricas são normalizadas antes do treinamento dos modelos de \textit{deep learning}.
...
Em cada etapa da validação temporal deslizante (\textit{walk-forward}), o normalizador é ajustado (fit)
...
Esse fluxo está organizado como \textit{pipeline} no script \texttt{prepare\_sequences.py}.
...
\begin{itemize}
  ...
  \item \textbf{Bandas de Bollinger}: período 20 e 2 desvios-padrão \cite{Bollinger2001}.
        ...
        (i) a largura da banda e (ii) a posição relativa do preço.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Viabilidade amostral}: cada nova sequência consome 60 barras de histórico;
        quanto maior a janela retrospectiva (lookback), menor o número de amostras disponíveis.
\end{itemize}

No \textit{pipeline}, o valor padrão está definido em \texttt{config.py}
(\texttt{JANELA\_TEMPORAL\_STEPS = 60}).
Além da janela padrão (60), foram avaliadas janelas alternativas (16, 32 e 64 barras) por meio de otimização bayesiana (Optuna), a fim de verificar sensibilidade do desempenho à janela retrospectiva (lookback).

% ---------------------------------------------------------------------------
\section{Modelos — modelos de referência e proposto}
% ---------------------------------------------------------------------------
...
\subsection{Modelos de referência e justificativa.} Os modelos de referência (\textit{baselines}) servem para estabelecer um piso de desempenho e verificar se modelos mais complexos trazem ganho real sobre regras simples e métodos clássicos. 
...
\begin{itemize}
  ...
  \item \textit{\textbf{LSTM puro}}: uma camada LSTM seguida de camada densa com sigmoide; \textit{baseline} de \textit{deep learning} que usa apenas dependências temporais de longo prazo, sem estágio convolucional \cite{hochreiter_1997}. Está implementado no \textit{pipeline} para análises complementares; a tabela comparativa principal (Capítulo~5) reporta o modelo proposto contra Naive, Drift, ARIMA e Prophet.
\end{itemize}
...
% --- Subseção: Protocolo temporal (\textit{walk-forward}) e particionamento
\subsection{Protocolo temporal (validação temporal deslizante (\textit{walk-forward})) e particionamento}
O histórico de cada ativo é dividido em \textbf{blocos sequenciais}, seguindo validação
validação temporal deslizante (\textit{walk-forward}) \cite{lopezdeprado2018,bergmeir2012}.
...
Com o volume efetivo do conjunto de dados (em torno de 36 mil barras por ativo), o número de folds é determinado
automaticamente pela extensão da série; nos experimentos deste trabalho resultaram \textbf{4 folds} por ativo
(quatro blocos completos treino--embargo--teste no período disponível).
...
O \textit{pipeline} completo é executado por \texttt{rodar\_pipeline\_completo.sh}
...
Os modelos de \textit{deep learning} (CNN--LSTM e LSTM) são treinados por fold,
...
e o \textit{pipeline} é orquestrado por \texttt{rodar\_pipeline\_completo.sh}
\cite{loshchilov2019}.
...
e complementamos com \textit{backtests} para traduzir acerto estatístico em impacto financeiro.
...
(taxa de acerto (\textit{hit rate})): a proporção de vezes em que a previsão de \emph{alta} ou \emph{baixa}
...
e reduz a influência de movimentos muito pequenos, onde o \textit{spread} e o \emph{bid--ask bounce}
podem dominar \cite{hasbrouck2007empirical}.

Para dar contexto além do taxa de acerto (\textit{hit rate}), reportamos também métricas clássicas de classificação:
...
Por fim, como acerto direcional não garante lucro, avaliamos a utilidade prática com \textit{backtests}.
Neles, incluímos custos de transação e deslizamento de preço (\textit{slippage}) explícitos.
Reportamos então retorno líquido, índice de Sharpe, \textit{max drawdown} e fator de lucro (\textit{profit factor})
\cite{sharpe1994}.
...
Para medir qualidade probabilística, a metodologia prevê \textbf{Log-Loss} e \textbf{Brier Score}.
...
\cite{brier1950}.
Na implementação atual do \textit{pipeline}, essas métricas não foram computadas nem reportadas nos resultados;
sua inclusão permanece como extensão natural para trabalhos futuros.
...
As métricas são calculadas \textbf{por fold} do validação temporal deslizante (\textit{walk-forward}),
isto é, em cada bloco de teste.
Depois, agregamos os resultados entre folds para obter uma visão mais estável do desempenho.
A regra padrão do \textit{pipeline} é usar a \textbf{média} das métricas ao longo dos folds,
...
Por fim, o script \texttt{comparar\_modelos.py} consolida CNN--LSTM e modelos de referência
...
% ---------------------------------------------------------------------------
\section{\textit{Backtests} e custos}
\label{sec:backtests-custos}
% ---------------------------------------------------------------------------

% --- Subseção: Regras do backtest
\subsection{Regras do backtest.} Foram executados \textit{backtests} em duas modalidades: \textbf{\textit{long-only}} (apenas compra ou neutro; sinal de baixa é tratado como saída para caixa) e \textbf{\textit{long/short}} (compra, venda a descoberto ou neutro). A entrada e a saída são determinadas pelas probabilidades previstas pelo modelo: dado um limiar (por padrão 0,5), probabilidade acima do limiar gera sinal de alta (1), abaixo de $(1 - \text{limiar})$ gera sinal de baixa ($-1$), e no intervalo intermediário o sinal é neutro (0). O backtest é aplicado por fold: para cada bloco de teste do validação temporal deslizante (\textit{walk-forward}), os retornos realizados são alinhados ao sinal previsto (retorno na barra seguinte à previsão) e a curva de capital é simulada período a período, com custos descontados a cada mudança de posição. A implementação está em \texttt{utils/backtesting.py} e é acionada pelo script \texttt{rodar\_backtest.py} por ativo, fold e estratégia \cite{b3_servicos}.

% --- Subseção: Custos e \textit{slippage}
\subsection{Custos e deslizamento de preço (\textit{slippage}).} Os custos são modelados em três componentes: (i) \textbf{custo fixo} por operação (corretagem em R\$; no pipeline padrão 0, conforme corretoras com corretagem zero); (ii) \textbf{custo proporcional} ao volume negociado, fixado em 0,03\% (\texttt{CUSTO\_TAXA\_PROPORCIONAL = 0.0003}), em linha com emolumentos B3 (Neg, CCP, TTA) para operações regulares; (iii) \textbf{deslizamento de preço (\textit{slippage})} como fração do valor, em 0,05\% (\texttt{CUSTO\_SLIPPAGE = 0.0005}), faixa realista entre 0,05\% e 0,1\%. O capital inicial padrão é R\$~100.000 (\texttt{CAPITAL\_INICIAL}). Cada mudança de posição (entrada ou saída) incorre em custo; no caso de reversão (long $\leftrightarrow$ short), contam-se duas pernas. A análise de \textbf{sensibilidade a custos} varia os parâmetros por multiplicadores (0,5$\times$, 1$\times$, 1,5$\times$, 2$\times$) para verificar como retorno líquido, Sharpe e rotatividade (\textit{turnover}) respondem ao aumento de atrito.

% --- Subseção: Métricas do backtest
\subsection{Métricas do backtest.} Para cada execução são reportados: \textbf{retorno líquido} (variação percentual do capital inicial ao final, após todos os custos); \textbf{índice de Sharpe} anualizado (média dos retornos líquidos por período sobre desvio padrão, multiplicado por $\sqrt{\text{barras\_por\_ano}}$); \textbf{\textit{max drawdown}} (maior queda percentual do pico ao vale na curva de capital); \textbf{\textit{profit factor}} (soma dos ganhos / valor absoluto da soma das perdas); \textbf{\textit{turnover}} (número de mudanças de posição por barra) e \textbf{número de operações}. Os resultados são salvos em CSV em \texttt{data/backtest/} por ativo, fold e estratégia.
...
A reprodutibilidade é garantida através de sementes fixas, versionamento de dependências e um \textit{pipeline} automatizado que executa todas as etapas em sequência.

% --- Subseção: Organização do repositório e versões
\subsection{Organização do repositório e versões.}
O código-fonte está organizado no diretório \texttt{codigo/pipeline/}, seguindo uma estrutura modular que separa responsabilidades. O repositório com o código isolado do \textit{pipeline} está disponível em: \url{https://github.com/rafaelmelo2/TCC/tree/pipeline-only}.

\begin{itemize}
  \item \texttt{src/data\_processing/}: módulos para carregamento, validação e engenharia de features;
  \item \texttt{src/models/}: implementações dos modelos (\textit{baselines} e CNN--LSTM);
  \item \texttt{src/utils/}: métricas, validação \textit{walk-forward}, \textit{backtesting} e otimização com Optuna;
  \item \texttt{src/scripts/}: scripts de execução para análises e visualizações;
  \item \texttt{src/tests/}: testes dos modelos de referência e análises de sensibilidade;
...
(2) treinamento dos modelos de referência,
(3) treinamento do CNN--LSTM com Optuna,
(4) análise dos modelos salvos,
(5) execução de \textit{backtests},
(6) testes estatísticos (Diebold--Mariano) e
(7) análise de sensibilidade do \textit{walk-forward}.
...
\begin{enumerate}
  ...
  \item \textbf{Execução do \textit{pipeline} completo}: Execute o script principal a partir do diretório \texttt{codigo/pipeline/}:
        \begin{itemize}
        ...
        \begin{itemize}
          ...
          \item Executa \textit{backtests} em \texttt{data/backtest/};
...
\begin{itemize}
  ...
  \item Testar todos os modelos de referência com \textit{walk-forward}:
        \texttt{uv run python src/tests/testar\_baselines\_walkforward.py --todos}
\end{itemize}

Todos os artefatos gerados (modelos, métricas, \textit{backtests}, logs) são salvos com timestamps e estruturas de diretórios consistentes, facilitando a análise posterior e a comparação entre diferentes execuções.
