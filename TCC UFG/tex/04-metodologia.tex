...
Os dados foram obtidos via plataforma MetaTrader~5 (MT5), com histórico consolidado e marcação temporal (timestamping) consistente no fuso do pregão \cite{b3_servicos}. A escolha dos ativos não foi arbitrária: o universo contempla \textbf{três ativos} que figuram entre os de maior relevância e volume de negociação na B3 — PETR4 (Petrobras), VALE3 (Vale) e ITUB4 (Itaú Unibanco). Esses papéis estão entre os mais líquidos e mais movimentados da bolsa brasileira, com participação relevante no índice Ibovespa e em indicadores de volume, o que os torna adequados para estudos intradiários: além de reduzir spreads, gaps e deslizamento de preço (slippage), ativos mais movimentados tendem a oferecer séries temporais mais estáveis e menos sujeitas a artefatos de microestrutura em testes retrospectivos (backtests). Os ativos foram mantidos fixos durante todo o período para mitigar viés de sobrevivência (survivorship bias).
...
\subsection{Modelos de referência e justificativa.} Os modelos de referência (baselines) servem para estabelecer um piso de desempenho e verificar se modelos mais complexos trazem ganho real sobre regras simples e métodos clássicos. 
...
\begin{itemize}
  ...
  \item \textit{\textbf{LSTM puro}}: uma camada LSTM seguida de camada densa com sigmoide; baseline de deep learning que usa apenas dependências temporais de longo prazo, sem estágio convolucional \cite{hochreiter_1997}. Está implementado no pipeline para análises complementares; a tabela comparativa principal (Capítulo~5) reporta o modelo proposto contra Naive, Drift, ARIMA e Prophet.
\end{itemize}
...
\cite{brier1950}.
Na implementação atual do pipeline, essas métricas não foram computadas nem reportadas nos resultados;
...
O código-fonte está organizado no diretório \texttt{codigo/pipeline/}, seguindo uma estrutura modular que separa responsabilidades. O repositório com o código isolado do pipeline está disponível em: \url{https://github.com/rafaelmelo2/TCC/tree/pipeline-only}.
...
\begin{itemize}
  ...
  \item Testar todos os modelos de referência com walk-forward:
